...
import sysconfig
NVRTC_LIB_PATHS = [f'/{pre}/cuda/targets/{tgt}/lib' for pre in ['opt', 'usr/local']
                   for tgt in [sysconfig.get_config_vars().get("MULTIARCH", "").rsplit("-", 1)[0], 'sbsa-linux']] + \
                  [f'{sysconfig.get_path("purelib")}/nvidia/cuda_nvrtc/lib']
dll = c.DLL('nvrtc', 'nvrtc', NVRTC_LIB_PATHS)
